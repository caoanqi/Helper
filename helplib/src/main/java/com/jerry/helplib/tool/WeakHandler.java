 ...
 * Handler的原始实现始终保持对执行队列中的处理程序的硬引用。 *如果您创建匿名处理程序并将延迟消息发布到其中，即使可以清除它，它也会将所有父类*保留在内存中。
 * 这个实现比较棘手，它会将WeakReferences保留给runnables和消息，*和一旦WeakHandler实例不被引用，GC就可以收集它们
 *
 * @see Handler
 * <p>
 * fileName:
